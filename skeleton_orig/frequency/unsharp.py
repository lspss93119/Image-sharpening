...
#
fsmunsharp = fshift*2 -fsmimg
# fft 結果是複數，其絕對值結果是振幅
rimg = np.log(np.abs(fsmunsharp))


# 傅立葉逆變換
ishift = np.fft.ifftshift(fsmunsharp)
iimg = np.fft.ifft2(fsmunsharp)
iimg = np.abs(iimg)

# 
cv2.imwrite('moon_unsharp.jpg',iimg)
# 展示結果
